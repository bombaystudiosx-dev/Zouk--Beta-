...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          zouk_user_profile: this._safeGetItem('zouk_user_profile'),
          zouk_settings: this._safeGetItem('zouk_settings'),
          zouk_profile: this._safeGetItem('zouk_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('zouk_viewed_features'),
          developer_mode: this._safeGetItem('zouk_developer_mode'),
        ...
        ui: {
          // Tab configuration
          zouk_tab_configuration: this._safeGetItem('zouk_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('zouk_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('zouk_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          zouk_read_logs: this._safeGetItem('zouk_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('zouk_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('zouk_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.zouk_tab_configuration) {
        try {
          this._safeSetItem('zouk_tab_configuration', data.ui.zouk_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'zouk_acknowledged_debug_issues',
        'zouk_acknowledged_connection_issue',
        'error_logs',
        'zouk_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('zouk_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
