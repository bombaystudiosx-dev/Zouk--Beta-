...
export function simplifyZoukActions(input: string): string {
  // Using regex to match zoukAction tags that have type="file"
  const regex = /(<zoukAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/zoukAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<zoukAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</zoukAction>`;
    });

  return `<zoukArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</zoukArtifact>`;
}
