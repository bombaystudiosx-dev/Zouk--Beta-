...
function sanitizeText(text: string): string {
  let sanitized = text.replace(/<div class=\\"__zoukThought__\\">.*?<\/div>/s, '');
  sanitized = sanitized.replace(/<think>.*?<\/think>/s, '');
  sanitized = sanitized.replace(/<zoukAction type="file" filePath="package-lock\.json">[\s\S]*?<\/zoukAction>/g, '');
