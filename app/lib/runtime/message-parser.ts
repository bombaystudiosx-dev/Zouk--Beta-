import type { ActionType, ZoukAction, ZoukActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { ZoukArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<zoukArtifact';
const ARTIFACT_TAG_CLOSE = '</zoukArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<zoukAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</zoukAction>';
const ZOUK_QUICK_ACTIONS_OPEN = '<zouk-quick-actions>';
const ZOUK_QUICK_ACTIONS_CLOSE = '</zouk-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends ZoukArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: ZoukArtifactData;
  currentAction: ZoukActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(ZOUK_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(ZOUK_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + ZOUK_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <zouk-quick-action ...>label</zouk-quick-action> inside
          const quickActionRegex = /<zouk-quick-action([^>]*)>([\s\S]*?)<\/zouk-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + ZOUK_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as ZoukAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as ZoukAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__zoukQuickAction__"',
    'data-zouk-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__zoukQuickAction__\" data-zouk-quick-action=\"true\">${buttons.join('')}</div>`;
}
