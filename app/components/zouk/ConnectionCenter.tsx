  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <span style={{ fontSize: 11, color: '#6a6a6a', display: 'block', marginTop: 1 }}>{connector.description}</span>
      </div>
      <button
        onClick={connected ? connector.disconnect : connector.connect}
        style={{
          padding: '4px 12px',
          background: connected ? 'transparent' : 'rgba(236,29,46,0.10)',
          border: `1px solid ${connected ? '#2a2a2a' : '#ec1d2e'}`,
          borderRadius: 6,
          color: connected ? '#6a6a6a' : '#ec1d2e',
          fontSize: 11,
          fontWeight: 600,
          cursor: 'pointer',
          whiteSpace: 'nowrap',
...
export function ConnectionCenter({ open, onClose, anchorRef }: Props) {
  ...
  const grouped = CONNECTOR_REGISTRY.reduce<Record<string, Connector[]>>((acc, c) => {
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 11, color: '#6a6a6a', margin: '2px 0 0' }}>
            {CONNECTOR_REGISTRY.filter((c) => c.status === 'connected').length} of {CONNECTOR_REGISTRY.length} connected
          </p>
        </div>
        <button
          onClick={onClose}
