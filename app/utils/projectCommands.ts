...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<zoukAction type="shell">${commands.setupCommand}</zoukAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<zoukAction type="start">${commands.startCommand}</zoukAction>
`;
  }
...
<zoukArtifact id="project-setup" title="Project Setup">
${commandString}
</zoukArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeZoukArtifactTags(input: string) {
  // Regular expression to match zoukArtifact tags and their content
  const regex = /(<zoukArtifact[^>]*>)([\s\S]*?)(<\/zoukArtifact>)/g;
...
export function escapeZoukAActionTags(input: string) {
  // Regular expression to match zoukArtifact tags and their content
  const regex = /(<zoukAction[^>]*>)([\s\S]*?)(<\/zoukAction>)/g;
...
export function escapeZoukTags(input: string) {
  return escapeZoukArtifactTags(escapeZoukAActionTags(input));
}
