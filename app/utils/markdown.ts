  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__zoukThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__zoukArtifact__', '__zoukThought__', '__zoukQuickAction', '__zoukSelectedElement__'],

      // ['className', '__zoukThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__zoukArtifact__', '__zoukThought__', '__zoukQuickAction'],
    ],
  },
  strip: [],
};
