...
[![zouk.diy: AI-Powered Full-Stack Web Development in the Browser](./public/social_preview_index.jpg)](https://zouk.diy)
...
Let's get you up and running with the stable version of Zouk.DIY!

## Quick Installation

[![Download Latest Release](https://img.shields.io/github/v/release/stackblitz-labs/zouk.diy?label=Download%20Zouk&sort=semver)](https://github.com/stackblitz-labs/zouk.diy/releases/latest) ← Click here to go to the latest release version!
...
You have two options for running Zouk.DIY: directly on your machine or using Docker.
...
2. **Build an Image**
   ...
   # ≈ docker build -t zouk-ai:development -t zouk-ai:latest --target development .

   # Production image (self-contained build artifacts)
   pnpm run dockerbuild:prod
   # ≈ docker build -t zouk-ai:production -t zouk-ai:latest --target zouk-ai-production .
   ```
...
3. **Run the Container**
   ...
   docker run --rm -p 5173:5173 --env-file .env.local zouk-ai:latest
   ```
...
For users who prefer a native desktop experience, zouk.diy is also available as an Electron desktop application:

1. **Download the Desktop App**:
   - Visit the [latest release](https://github.com/stackblitz-labs/zouk.diy/releases/latest)
...
Zouk.diy features a modern, intuitive settings interface for managing AI providers and API keys. The settings are organized into dedicated panels for easy navigation and configuration.
...
Zouk.diy supports both methods for maximum flexibility:
...
1. **Clone the Repository**:
   ...
   ```bash
   git clone -b stable https://github.com/stackblitz-labs/zouk.diy.git
   ```
...
>     - Explanation: The System Prompt currently implemented in zouk.diy cant cover the best performance for all providers and models out there. So it works better with some models, then other, even if the models itself are perfect for >programming
